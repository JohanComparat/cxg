...
# p
# plot wprp
p2_fig = os.path.join( fig_dir, 'wprp-obs-M1025-ANY-BC-RS-uchuu.png')
plt.figure(11, (6,5))
t_wp = BGS["ANY_10.25"]
plt.plot(t_wp['rp_mid'], t_wp['rp_mid']*t_wp['wprp'], lw=4,  ls='solid', label='All galaxies', color='grey')
t_wp = BGS["RS_10.25"]
plt.plot(t_wp['rp_mid'], t_wp['rp_mid']*t_wp['wprp'], lw=4,  ls='solid', label='RS', color='darkred')
t_wp = BGS["BC_10.25"]
plt.plot(t_wp['rp_mid'], t_wp['rp_mid']*t_wp['wprp'], lw=4,  ls='solid', label='BC', color='darkblue')

t_wp = U0p14["ANY_10.25"]
...
plt.legend(loc=4, fontsize=12, title='0.05<z<0.22,\n' +r'10.25$<\log_{10}(M*/M_\odot)<$12')
plt.tight_layout()
plt.savefig(p2_fig)
plt.clf()
print(p2_fig)
...
#plt.yscale('log')
#plt.xlabel(r"$r_p$ [Mpc/h]")
#plt.ylabel(r"$w_p(r_p)$ [Mpc/h], $\pi_{max}=100$ Mpc$/h$")
...
plt.plot(t_wp['rp_mid'], t_wp['wprp']*t_wp['rp_mid'], lw=1,  ls='solid', label='Galaxies x Clusters ', color='grey')
...
b_clu_e = 0.21
xx = t_wp['rp_mid']
plt.fill_between(xx, y1=np.ones_like(xx)*( ((b_clu-b_clu_e)/b_gal)), y2=np.ones_like(xx)*( ((b_clu+b_clu_e)/b_gal)), color='darkgreen', alpha=0.3)
