...
p2_all_dat  = np.array(glob.glob(os.path.join(dat_dir, 'Counts_Euclid_'+cl_sample+'_RAND_gr_*_z_*.RSdata.npy')))
...
RS_model = Table.read( '/home/comparat/sf_Shared/data/eromapper/models/colors/ezgal.euc_VisYJH.speclite.bc03_exp_0.1_z_0.02_salp.model.cosmo.esutil.z3.fit')
z_RS = np.hstack(( RS_model['Z'] ))
gr_RS = np.hstack(( np.array(RS_model['COLOR']).T[0] ))
#gr_RS_sigma = np.hstack(( np.array(RS_model['meancol_scatter']).T[0].T[0] ))

# plot CT

p2_fig  = os.path.join(fig_dir, 'RS_vy_z_SN100kpc_'+cl_sample+'_Euclid.png')
...
p2_fig  = os.path.join(fig_dir, 'RS_vy_z_SN1Mpc_'+cl_sample+'_Euclid.png')
plt.figure(11, (7.4,7.5))
