...
for jj, p2_data in enumerate(p2_all_dat[:500]):
    ...
    p2_fig  = os.path.join(fig_dir, 'RS_gr_z_intWTH_'+os.path.basename(p2_data)+'.png')
    ...
    #plt.plot(RES[jj]['color_grid']+color_step/2., RES[jj]['intWTH_100kpc'], ls='', color='darkgreen', marker='+')#, xerr=color_step/2., yerr=[RES[jj]['intWTH_100kpc']-RES[jj]['intWTH_100kpc_lo'], RES[jj]['intWTH_100kpc_up']-RES[jj]['intWTH_100kpc']], label=r'$R_{max}=200kpc$', ls='', lw=1, c='darkgreen')
    #plt.fill_between(RES[jj]['color_grid']+color_step/2., RES[jj]['intWTH_100kpc_lo'], RES[jj]['intWTH_100kpc_up'], color='darkgreen', alpha=0.3)
    plt.plot(RES[jj]['color_grid'][s1]+color_step/2., RES[jj]['intWTH_1Mpc'][s1], label=r'DATA ', ls='', marker='x', color='darkred') #  , xerr=color_step/2., yerr=[RES[jj]['intWTH_1Mpc'][s1]-RES[jj]['intWTH_1Mpc_lo'][s1], RES[jj]['intWTH_1Mpc_up'][s1]-RES[jj]['intWTH_1Mpc'][s1]])
    plt.fill_between(RES[jj]['color_grid'][s1]+color_step/2., RES[jj]['intWTH_1Mpc_lo'][s1], RES[jj]['intWTH_1Mpc_up'][s1], color='darkred', alpha=0.3)
    #plt.axvline(x_loc, label=r'$R_{max}=500kpc$, g-r=' +str(np.round(x_loc,3)), c='darkred' , ls='--', lw=2)
    #plt.axvline(RES[jj]['color_grid'][i_max3]+color_step/2., label=r'$R_{max}=200kpc$, g-r='+str(np.round(RES[jj]['color_grid'][i_max3]+color_step/2.,3)), c='darkgreen' , ls='--', lw=2)
    x_values = np.arange(0,2,0.001) #RES[jj]['color_grid'].min()+color_step/2.
    scale = RS_interp_lsigma(RES[jj]['z_bar'])
    y_values = norm.pdf(x_values, loc=RS_interp(RES[jj]['z_bar']), scale=scale)
    plt.plot(x_values, y_values*np.max(RES[jj]['intWTH_1Mpc'][s1])/y_values.max(), color='k', label='model Kluge 24, g-r='+str(np.round(RS_interp(RES[jj]['z_bar']),3)) + r', $\sigma=$'+str(np.round(scale,3)))
    #plt.axvline(RS_interp(RES[jj]['z_bar']), color='k', lw=2, ls='--',  )
    #plt.axvline(RS_interp_up(RES[jj]['z_bar']), color='k', lw=2, ls=':')
    #plt.axvline(RS_interp_lo(RES[jj]['z_bar']), color='k', lw=2, ls=':')
    fun = lambda c_val, c_0, c_m, c_scale, c_loc : ( norm.pdf(c_val, loc=c_loc, scale=c_scale) + c_0 ) * c_m
    ...
    y_values = fun(x_values, p_out[0], p_out[1], p_out[2], p_out[3])
    plt.plot(x_values, y_values*np.max(RES[jj]['intWTH_1Mpc'][s1])/y_values.max(), label=r'fit g-r='+str(np.round(p_out[3],3)) + ', $\sigma=$'+str(np.round(p_out[2],3)), color='r' )
    #scale = 0.2
    #y_values = norm.pdf(x_values, loc=x_loc, scale=scale)
    #plt.plot(x_values, y_values*np.max(RES[jj]['intWTH_1Mpc'][s1])/y_values.max(), label=r'$\sigma=$'+str(np.round(scale,2)))
    plt.xlabel(r'color $g-r$')
    plt.xlim((RS_interp(RES[jj]['z_bar'])-0.5,RS_interp(RES[jj]['z_bar'])+0.5))
    plt.ylabel(r'$w_{500kpc}$')
    #plt.yscale('log')
    ...
    plt.plot(RES[jj]['color_grid'][s1]+color_step/2., RES[jj]['intWTH_1Mpc'][s1]/y_model, label=r'DATA / model', ls='--', marker='o', color='darkred')
    ...
    plt.plot(RES[jj]['color_grid'][s1]+color_step/2., RES[jj]['intWTH_1Mpc'][s1]/y_MK24, label=r'DATA / model K24', ls='--', marker='s', color='k')
    plt.axvline(p_out[3], color='darkred', lw=2, ls='--',  )
    plt.axvline(p_out[3]+p_out[2], color='darkred', lw=2, ls=':')
    plt.axvline(p_out[3]-p_out[2], color='darkred', lw=2, ls=':')
    plt.xlabel(r'color $g-r$')
    plt.xlim((RS_interp(RES[jj]['z_bar'])-0.5,RS_interp(RES[jj]['z_bar'])+0.5))
    ...
    #plt.yscale('log')
    #plt.title('clusters '+cl_sample +',z='+str(np.round(RES[jj]['z_bar'],3)))
    plt.legend(loc=3, fontsize=10, framealpha=0.6)
    plt.tight_layout()
    plt.savefig(p2_fig)
    plt.clf()
    print(p2_fig)
...
plt.scatter(RS['all_z'][ok], RS['all_colors'][ok], s=100, marker='s', c=np.log10(RS['intWTH_1Mpc'][ok]) ,  label='C2 x BGS', alpha=0.5, linewidths=0)#, vmax=15) cmap=pl.cm.cool_r,
plt.plot(z_RS, gr_RS, 'k--',lw=2,label='model Kluge 24')
plt.plot(z_RS, gr_RS-gr_RS_sigma, 'k:', lw=2)
plt.plot(z_RS, gr_RS+gr_RS_sigma, 'k:', lw=2)
plt.plot(params_WT[-1], params_WT[3], 'r--', lw=2,label=r'fit, this work',c='darkred')
